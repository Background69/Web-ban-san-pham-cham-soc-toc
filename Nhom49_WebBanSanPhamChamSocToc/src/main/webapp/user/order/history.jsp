                                ...
                                <c:forEach var="item" items="${order.items}" varStatus="loop">
                                    <c:if test="${loop.index < 2}">
                                        <div class="order-item">
                                            <img src="${pageContext.request.contextPath}/static/images/${item.productImage != null ? item.productImage : 'default-product.png'}"
                                            ...
                                            <span class="item-price"><fmt:formatNumber value="${item.subtotal}" type="number"/>₫</span>
                                        </div>
                                    </c:if>
                                </c:forEach>
                                <c:if test="${order.items.size() > 2}">
                                    <p class="more-items">+ ${order.items.size() - 2} sản phẩm khác</p>
                                </c:if>
                            </div>
                            ...
                            <div class="order-footer">
                                ...
                                <div class="order-actions">
                                    ...
                                    <c:if test="${order.status == 'COMPLETED'}">
                                        <a href="${pageContext.request.contextPath}/orders/${order.orderId}/review" class="btn-review">
                                            Đánh giá
                                        </a>
                                    </c:if>
                                </div>
