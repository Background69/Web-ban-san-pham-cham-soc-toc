...
<head>
    ...
    <style>
        ...
        .modal-content{background:#fff;width:min(720px,92vw);margin:60px auto;border-radius:12px;position:relative;overflow:hidden;box-shadow:0 20px 60px rgba(0,0,0,.25)}
        ...
        .form-group input,.form-group select,.form-group textarea{width:100%;padding:10px 12px;border:1px solid #cfcfcf;border-radius:8px;outline:none}
        ...
        .thumb{width:52px;height:52px;object-fit:cover;border-radius:8px;border:1px solid #eee}
...
<div class="container">
    ...
    <main class="content">
        ...
        <c:if test="${param.err == '1'}">
            <div style="margin:10px 0;padding:10px 12px;border-radius:8px;background:#ffebee;color:#c62828;font-weight:700">
                ...
                <tr>
                    ...
                    <td>
                        <c:if test="${not empty p.primaryImageUrl}">
                            <img class="thumb" src="${pageContext.request.contextPath}/static/assets/${p.primaryImageUrl}" alt="thumb">
                        </c:if>
                    </td>

                    <td><c:out value="${p.productName}"/></td>
                    <td><c:out value="${p.productSlug}"/></td>
                    ...
                    <td>
                        <c:choose>
                            <c:when test="${p.defaultVariant != null && p.defaultVariant.salePrice != null && p.defaultVariant.salePrice > 0}">
                                <fmt:formatNumber value="${p.defaultVariant.salePrice}" type="number"/> ₫
                            </c:when>
                            <c:when test="${p.defaultVariant != null && p.defaultVariant.originalPrice != null && p.defaultVariant.originalPrice > 0}">
                                <fmt:formatNumber value="${p.defaultVariant.originalPrice}" type="number"/> ₫
                            </c:when>
...
<div id="productModal" class="modal" onclick="backdropClose(event)">
    <div class="modal-content">
        ...
        <form action="${pageContext.request.contextPath}/admin/products" method="post" enctype="multipart/form-data" style="margin:0;">
...
<script>
    ...
    function backdropClose(e){if(e.target&&e.target.classList.contains('modal')) closeModal();}
