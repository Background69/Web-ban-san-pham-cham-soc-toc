...
public class ProductDAO implements IDAO<Product> {
    ...
    public List<Product> findOnSale() {
        String sql = "SELECT * FROM products WHERE is_on_sale = true AND is_deleted = FALSE ORDER BY created_at DESC";
        return jdbi.withHandle(handle -> handle.createQuery(sql)
                .map((rs, ctx) -> mapProduct(rs))
                .list());
    }
    ...
    public List<Product> findOnSale(int limit, int offset) {
        String sql = "SELECT * FROM products WHERE is_on_sale = true AND is_deleted = FALSE ORDER BY created_at DESC LIMIT :limit OFFSET :offset";
        return jdbi.withHandle(handle -> handle.createQuery(sql)
                .bind("limit", limit)
                .bind("offset", offset)
                .map((rs, ctx) -> mapProduct(rs))
    ...
    public int countOnSale() {
        String sql = "SELECT COUNT(*) FROM products WHERE is_on_sale = true AND is_deleted = FALSE";
        return jdbi.withHandle(handle -> handle.createQuery(sql)
                .mapTo(Integer.class)
                .findFirst()
                .orElse(0));
